...
Sentinel SHALL load configuration from YAML or JSON files, MUST apply environment variable overrides using the `SENTINEL_` prefix, SHALL support environment-only configuration for full runtime operation (including structured fields), SHALL validate expanded event type values including wildcard `*` for route event matching, SHALL validate sink type-specific configuration for supported sink types including `discord`, SHALL validate notifier device-target selector fields (`notifier.routes[].device.names`, `notifier.routes[].device.tags`, `notifier.routes[].device.owners`, `notifier.routes[].device.ips`), SHALL validate Tailscale onboarding fields including advertise tags and OAuth credential combinations, and operator documentation SHALL include working configuration examples that reflect this behavior.
